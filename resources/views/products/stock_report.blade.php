...
<div class="w-full flex flex-col space-y-6">
    ...
    <div class="bg-white rounded shadow p-4">
        <form method="GET" action="{{ route('product.stock.report') }}" class="flex flex-wrap gap-4 items-end">
            <div>
            ...
            @if(request()->filled('start_date') || request()->filled('end_date') || request()->filled('status') || request()->filled('category_id'))
                <a href="{{ route('product.stock.report') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 font-medium">
                    Clear
                </a>
            @endif
            <div class="flex gap-2">
                <a href="{{ route('product.stock.filter.pdf', request()->query()) }}" target="_blank" class="bg-green-600 text-white px-4 py-2 rounded">Download PDF</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white shadow rounded p-4">
        <div class="overflow-x-auto">
            <h3 class="text-lg font-semibold text-gray-800">Product Stocks</h3>
                        ...
                        <tr class="border-t hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2">
                                ...
                                @if(auth()->user()->role === 'admin')
                                    @if($p->status === 'pending')
                                        {{-- Edit --}}
                                        <button type="button" onclick="openEditModal({{ $p->id }})" class="text-blue-600 hover:underline">Edit</button>
                                        {{-- Delete --}}
                                        <form action="{{ route('product.destroy',$p->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('.update-status-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            fetch("{{ route('product.stock.status.update') }}", {
                method: "POST",
                headers: {
