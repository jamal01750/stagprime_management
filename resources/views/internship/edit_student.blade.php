...
@section('title', 'Edit Intern | Intern Registration')
@section('heading', 'Edit Intern | Intern Registration')
    ...
    <div class="bg-white rounded shadow p-4 md:p-6 flex flex-col md:flex-row items-start">
        <div class="w-full flex flex-row">
            <div class="w-1/5 flex flex-col flex-shrink-0 pr-4">
                <h2 class="text-lg md:text-xl font-semibold mb-2 md:mb-4">Edit Intern</h2>
            </div>
            <div class="w-4/5">
                <form method="POST" action="{{ route('internship.update', $student->id) }}" enctype="multipart/form-data" class="space-y-3 md:space-y-4 w-full md:w-auto">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        <label class="block text-sm font-medium">Pay/Contract Amount</label>
                        <input type="number" name="pay_amount" value="{{ old('pay_amount', $student->pay_amount) }}" step="0.01" class="mt-1 block w-full border-2 border-blue-600 rounded" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium">Total Paid</label>
                        <input type="number" name="total_paid" value="{{ old('total_paid', $student->total_paid) }}" step="0.01" class="mt-1 block w-full border-2 border-blue-600 rounded" required>
                    </div>
