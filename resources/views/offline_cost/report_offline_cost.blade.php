...
@section('content')
    <div class="w-full flex flex-col space-y-6 md:space-y-8">
        ...
        <div class="bg-white rounded shadow p-4 md:p-6 mt-6">
            <form method="GET" action="{{ route('offline.cost.report') }}" class="flex space-x-4">
                <div>
        ...
        <div class="bg-white rounded shadow p-4 md:p-6 mt-6 overflow-x-auto">
            <h2 class="text-lg font-semibold mb-4">Monthly Expense and Due Date List</h2>
            <table class="w-full border-collapse border border-gray-300">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-left">Last/Due Date</th>
                ...
                <tbody>
                    @forelse($expenses as $expense)
                        <tr>
                            ...
                            <td class="border px-4 py-2">{{ $expense->last_date }}</td>
                            <td class="border px-4 py-2">{{ $expense->paid_date ?? '-' }}</td>
                            <td class="border px-4 py-2">
                                @if($expense->status === 'paid')
                                    <span class="text-green-600 font-medium">Paid</span>
                                @else
                                    <form action="{{ route('offline.cost.update', $expense->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
